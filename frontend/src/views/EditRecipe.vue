<template>
    ...
    <BRow class="justify-content-center w-100">
      <BCol cols="12" md="8" lg="7" class="mt-5"> <BCard title="Edit Recipe" class="shadow-lg p-4 my-5">
          <template #header>
            <h4 class="text-center mb-0">Edit Recipe</h4>
          </template>

          <BForm @submit.prevent="editRecipe">
            ...
            <BButton type="submit" variant="success" class="w-100 mb-3">Edit Recipe</BButton>
            <BButton variant="secondary" class="w-100" @click="cancelEdit">Cancel</BButton>
          </BForm>
...
<script>
import { updateRecipe, fetchSingleRecipe } from '../services/recipes';
import {
...
export default {
    ...
    async mounted() {
        ...
        try {
            const response = await fetchSingleRecipe(this.recipeId); 
            console.log('Fetched recipe:', response);
            this.recipe = response;
            this.cookingTime.hours = Math.floor(this.recipe.cookTime / 60);
            this.cookingTime.minutes = this.recipe.cookTime % 60;
            console.log('Cooking time:', this.cookingTime);
            console.log('cook time', this.recipe.cookTime);
        } catch (error) {
            console.error('Error fetching recipe:', error);
            this.errorMessage = 'Failed to fetch recipe details.';
            return;
        }
    },
    methods: {
        ...
        async editRecipe() {
            ...
            try {
                ...
                const response = await updateRecipe(this.recipeId,recipeData);
